...
class Box:
    def __init__(self, box_size = 16, num_particles = 131072, stop = 100, dt = 0.01, dt_get_states = 1, T = 1, mass = 200, d = 0.1, bounded = False, n_test=1, order = 2, offset_pos=False, offsets=(0.2,0.2,0.2)) -> None:
        ...
        for _ in tqdm(range(int(stop/dt))):
            ...
            self.handle_collisions(process_counts, grid, grid_sizes, self.particle_positions, self.particle_velocities, collision_rates_o0, collision_rates_o1, collision_rates_o2, np.float32(d), np.int32(box_size), np.int32(self.max_grid_size), np.float32(mass), np.float32(dt), np.int32(num_particles), collision_rand, np.int32(order), np.int32(self.bounded), block=(1024,1,1), grid=(self.num_grids//1024+1,1,1))
            ...
            if _ % dt_get_states == 0:
                ...
                self.cell_states(process_counts, grid, grid_sizes, self.particle_positions, self.particle_velocities, us, p_tensors, np.int32(box_size), np.int32(self.max_grid_size), block=(1024,1,1), grid=(self.num_grids//1024+1,1,1))
