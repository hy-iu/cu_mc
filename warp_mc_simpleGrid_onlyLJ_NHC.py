...
_kT = wp.static(wp.float64(TEMPERATURE))  # k_B = 1
...
FOURTH_ORDER_COEFFS = wp.static(wp.vec3d(
    1 / (2 - 2 ** (1 / 3)),
    -(2 ** (1 / 3)) / (2 - 2 ** (1 / 3)),
    1 / (2 - 2 ** (1 / 3)),
))

NHC_DELTA = wp.static(FOURTH_ORDER_COEFFS * _DT * 0.5 / wp.float64(NHC_N_LOOP))

@wp.kernel
def length_sq(v: wp.array(dtype=wp.vec3d), output: wp.array(dtype=wp.float64)): # pyright: ignore[reportInvalidTypeForm]
    pid = wp.tid()
    output[pid] = wp.length_sq(v[pid])
...
class NHCChainThermostat:
    def __init__(self):
        ...
        self.eta = np.zeros(NHC_M_CHAIN, dtype=np.float64)
        self.p_eta = np.zeros(NHC_M_CHAIN, dtype=np.float64)
    
    def nhc(self, v: wp.array(dtype=wp.vec3d)): # pyright: ignore[reportInvalidTypeForm]
        for _ in range(NHC_N_LOOP):
            for delta in NHC_DELTA:
                delta2 = delta / 2
                delta4 = delta / 4
                for j in range(NHC_M_CHAIN):
                    self._integrate_p_eta_j(v, NHC_M_CHAIN - j - 1, delta2, delta4)
                self.eta += delta * self.p_eta / self.Q
                v *= wp.float64(np.exp(-delta * self.p_eta[0] / self.Q[0]))
                for j in range(NHC_M_CHAIN):
                    self._integrate_p_eta_j(v, j, delta2, delta4)
    ...
    def _integrate_p_eta_j(self, v: wp.array(dtype=wp.vec3d), j: int, delta2: wp.float64, delta4: wp.float64): # pyright: ignore[reportInvalidTypeForm]
        ...
        if j == 0:
            ...
            total_v_sq = np.float64(wp.to_torch(v_sq).sum().item())
            g_j = np.float64(MASS) * total_v_sq - np.float64(3 * N * self._kT)
        else:
            g_j = self.p_eta[j - 1] * self.p_eta[j - 1] / self.Q[j - 1] - self._kT
...
class ParticleSystem:
    ...
    def evolve(self, output_dir=my_time_string()):
        ...
        i = pre_i = 0
        self.forces.zero_()
        nhc_thermostat = NHCChainThermostat()
        for t in tqdm(np.linspace(0, T_STOP, int(T_STOP / DT))):
            ...
            nhc_thermostat.nhc(self.velocities)
            if t % 0.1 < DT and i > 0:
                wp.launch(
                    kernel=length_f,
