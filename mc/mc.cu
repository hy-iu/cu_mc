...
__global__ void scatt_o0(float *rs,
  ...
  if (curand_uniform(&state) < collision_prob) {
    ...
    vs[_p1 * 3 + 1] += dy * dot;
    vs[_p1 * 3 + 2] += dz * dot;
    // rs[_p0 * 3 + 0] += vs[_p0 * 3 + 0] * dt;
    // rs[_p0 * 3 + 1] += vs[_p0 * 3 + 1] * dt;
    // rs[_p0 * 3 + 2] += vs[_p0 * 3 + 2] * dt;
    // rs[_p1 * 3 + 0] += vs[_p1 * 3 + 0] * dt;
    // rs[_p1 * 3 + 1] += vs[_p1 * 3 + 1] * dt;
    // rs[_p1 * 3 + 2] += vs[_p1 * 3 + 2] * dt;
  }
}
...
int main() {
  ...
  for (int step = 0; step < stop / dt; step++) {
    ...
    std::cout << "\rStep: " << step;
    update_positions<<<G, B>>>(rs, vs, dt * 0.03, &pressures[step], len_grid);
    // cudaDeviceSynchronize();
    ...
    for (int i = 0; i < 1; i++) {
      ...
      cudaDeviceSynchronize();
      scatt_o0<<<(n_grids * MAX_PARTICLES_PER_GRID / 2 + B - 1) / B, B>>>(rs, vs, grid_counts, grid_list, d, dt, T,
                                                                          mass, n_particles);
      // cudaMemcpy(&pressures[step], &temp, sizeof(float), cudaMemcpyHostToDevice);
      // update_positions<<<G, B>>>(rs, vs, dt, &pressures[step], len_grid);
      // compute_grid_indices<<<G, B>>>(flat_indices, rs, grid_counts, len_grid);
      // clear_grid_particle_counts<<<(n_grids + B - 1) / B, B>>>(grid_counts);
      // group_particles<<<G, B>>>(grid_counts, grid_list, flat_indices);
