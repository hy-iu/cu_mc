...
int main() {
    ...
    cudaMalloc((void **)&positions, sizeof(float) * 3 * num_particles);
    cudaMalloc((void **)&velocities, sizeof(float) * 3 * num_particles);
    cudaMalloc((void **)&speeds, sizeof(float) * num_particles);
    cudaMalloc((void **)&thetas, sizeof(float) * num_particles);
    cudaMalloc((void **)&phis, sizeof(float) * num_particles);
    cudaMalloc((void **)&offsets, sizeof(float) * 3);

    cudaMalloc((void **)&pressure_s, int(stop / dt) * sizeof(float));
    cudaMalloc((void **)&velocities_s, int(stop / dt) * sizeof(float));
    cudaMalloc((void **)&process_counts, int(stop / dt) * sizeof(int) * 3);
    cudaMalloc((void **)&grid, sizeof(int) * num_grids * max_grid_size);
    cudaMalloc((void **)&grid_sizes, sizeof(int) * num_grids);
    cudaMalloc((void **)&collision_rates_o0, sizeof(float) * num_grids);
    cudaMalloc((void **)&collision_rates_o1, sizeof(float) * num_grids);
    cudaMalloc((void **)&collision_rates_o2, sizeof(float) * num_grids);
    cudaMalloc((void **)&collision_rate_o0_s, sizeof(float) * int(stop / dt));
    cudaMalloc((void **)&collision_rate_o1_s, sizeof(float) * int(stop / dt));
    cudaMalloc((void **)&collision_rate_o2_s, sizeof(float) * int(stop / dt));
    ...
    for (int step = 0; step < int(stop / dt); step++) {
        ...
        update_positions<<<G, B>>>(process_counts + step, grid, grid_sizes, positions, velocities, pressure_s + step, offsets, (float)box_size,
            max_grid_size, mass, dt, num_particles, bounded, false);
        cudaDeviceSynchronize();
        float* collision_rand;
        cudaMalloc((void **)&collision_rand, sizeof(float) * num_grids * 2 * max_grid_size * max_grid_size);
        ...
        handle_collisions<<<(num_grids + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(process_counts + step, grid, grid_sizes, positions, velocities,
            ...
            d, (float)box_size, max_grid_size, mass, dt, num_particles, collision_rand, 2, bounded);
    ...
    //     cudaMemcpy(&h_velocity, velocities_s + step, sizeof(float), cudaMemcpyDeviceToHost);
        ...
        float t = step * dt;
        ps_file << t << ", " << h_pressure << ", " << h_process_counts[0] << ", " << h_process_counts[1] << ", " << h_process_counts[2] << std::endl;
    }
    ps_file.close();
    cudaFree(positions);
    cudaFree(velocities);
